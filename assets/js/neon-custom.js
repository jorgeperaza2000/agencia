...
;(function($, window, undefined){
	...
	$(document).ready(function()
	{
		...
		//Manejador del Menu... Para activar zona
		var menuController = $("#statusMenu").attr("data-controller");
		if ( menuController == "" ) {
			$("#dashboard").addClass("active");
		} else {
			$("#main-menu").children("li").removeClass("active");
			$("#"+menuController).addClass("active");
			$("#"+menuController).parents("li").addClass("active opened");
		}


		//CUENTA ATRAS PARA CERRAR LOS MODALES
		$.fn.countdown = function (callback, duration, message) {
	    // If no message is provided, we use an empty string
	    message = message || "";
	    // Get reference to container, and set initial content
	    var container = $(this[0]).html(message + duration);
